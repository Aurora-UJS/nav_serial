...
#ifndef RM_SERIAL_DRIVER__PROTOCOL__CRC_HPP_
#define RM_SERIAL_DRIVER__PROTOCOL__CRC_HPP_
...
inline uint8_t crc8_calculate(const uint8_t *data, size_t len, uint8_t init = 0xFF) {
  uint8_t crc = init;
...
inline bool crc8_verify(const uint8_t *data, size_t len, uint8_t expected_crc) {
  return crc8_calculate(data, len) == expected_crc;
}
...
inline uint16_t crc16_calculate(const uint8_t *data, size_t len, uint16_t init = 0xFFFF) {
  uint16_t crc = init;
  for (size_t i = 0; i < len; ++i) {
...
inline bool crc16_verify(const uint8_t *data, size_t len, uint16_t expected_crc) {
